...
class Experiment(ExperimentTTQ):
    ...
    def initial_magnitude_pruning(self, kernel, w_p, w_n):
        # Calculate weight magnitude
        weight_mag = torch.abs(kernel)
        
        # Calculate threshold based on weight magnitude
        threshold = torch.mean(weight_mag) + torch.std(weight_mag)
        
        # Apply pruning based on weight magnitude
    ...
    def gradient_guided_adaptive_pruning(self, kernel, w_p, w_n):
        ...
        # Calculate adaptive thresholds based on gradient magnitude
        grad_mean = torch.mean(grad_mag)
        grad_std = torch.std(grad_mag)
