...
class Experiment(ExperimentTTQ):
    ...
    def optimize_step(self, loss_value):
        ...
        decay_factor = 1 / (1 + self.update_counter * 0.0001)  # Gradual decay

        for i, (kernel, fp_kernel, (w_p, w_n), (a, b)) in enumerate(zip(
            quantized_kernels, fp_kernels, scaling_factors, thresholds
        )):
            # Compute gradients
            grad_fp, grad_wp, grad_wn, grad_a, grad_b, grad_alpha = self.get_grads(
                kernel.grad.data, fp_kernel.data, w_p.data, w_n.data
            )

            # Update full-precision kernel
            fp_kernel.grad = grad_fp
            ...
            kernel_norm = torch.norm(fp_kernel.data)
            importance_factor = kernel_norm / fp_kernel.data.numel()
            
            a_new = a.data - self.lr_thresh * grad_a * decay_factor * importance_factor
            b_new = b.data - self.lr_thresh * grad_b * decay_factor * importance_factor
            
            # Ensure a < b
            a.data, b.data = torch.min(a_new, b_new), torch.max(a_new, b_new)

            # Re-quantize the kernel
            kernel.data = self.quantize(fp_kernel.data, w_p.data, w_n.data)
