...
class Experiment(object):
    ...
    def compute_dataset_mean_std(self):
        """
            ...
            for normalization purposes, handling variable-sized images for UNet
        """
        if self.model_type.lower() == 'unet':
            # For UNet/Pascal VOC, we need to handle per-channel statistics
            # Initialize channel-wise sums and squared sums
            channel_sum = torch.zeros(3)  # RGB channels
            ...
            for batch, _ in self.train_loader:
                # batch shape: [B, C, H, W]
                channel_sum += torch.sum(batch, dim=[0, 2, 3])  # Sum across batch, height, width
                channel_sum_squared += torch.sum(batch ** 2, dim=[0, 2, 3])
                pixel_count += batch.shape[0] * batch.shape[2] * batch.shape[3]
            
            if self.separate_val_ds:
                for batch, _ in self.val_loader:
                    channel_sum += torch.sum(batch, dim=[0, 2, 3])
                    channel_sum_squared += torch.sum(batch ** 2, dim=[0, 2, 3])
                    pixel_count += batch.shape[0] * batch.shape[2] * batch.shape[3]

            # Calculate mean and std per channel
            self.dataset_mean = (channel_sum / pixel_count).numpy()
            # Using the formula: std = sqrt(E[X^2] - (E[X])^2)
            self.dataset_std = (torch.sqrt(channel_sum_squared / pixel_count - (channel_sum / pixel_count) ** 2)).numpy()
            
        else:
            # Original code for other models (MNIST etc.)
            # Creating dataloaders
            self.dataloadersCreation()
    ...
    def compute_iou(self, predictions, targets, smooth=1e-6):
        """
    ...
    def compute_dice_score(self, predictions, targets, smooth=1e-6):
        """
        Compute Dice score between predictions and targets
        """
        intersection = (predictions & targets).float().sum()
        dice = (2. * intersection + smooth) / (predictions.float().sum() + targets.float().sum() + smooth)
        return dice
    ...
    def dataloadersCreation(self):
        ...
        """
        if self.model_type.lower() == 'unet':
            # For segmentation, we might want to use different batch sizes
            ...
            # Original classification dataloader creation
            train_indices = list(range(0, len(self.train_ds)))
            train_sampler = SubsetRandomSampler(train_indices)
            self.train_loader = torch.utils.data.DataLoader(
                self.train_ds,
    ...
    def compute_forward_pass(self, batch, epoch_nb, keep_grad=True):
        if self.model_type.lower() == 'unet':
            # Getting the data and the labels for segmentation
            images, masks = batch
            ...
            iou = self.compute_iou(predictions, masks)
            dice_score = self.compute_dice_score(predictions, masks)
            
            metrics = {
                'IoU': iou.item(),
                'DiceScore': dice_score.item()
            }
            
            return loss, metrics
        else:
            # Original classification forward pass
            audio_features, labels = batch
            audio_features, labels = audio_features.to(self.device), labels.to(self.device)
            
            out = self.model(audio_features)
    ...
    def single_train(self):
        ...
        # Modified predictions structure for segmentation
        if self.model_type.lower() == 'unet':
            predictions_results = {}
            for dataset_split in ['Train', 'Val', 'Test']:
                predictions_results[dataset_split] = {
                    'IoU': [[] for _ in range(self.nb_epochs)],
                    'DiceScore': [[] for _ in range(self.nb_epochs)]
                }
        else:
            # Original classification predictions structure
            predictions_results = {}
            for dataset_split in ['Train', 'Val', 'Test']:
                predictions_results[dataset_split] = {}
        ...
        best_metric_so_far = -1  # Best IoU for segmentation, best MCC for classification

        for epoch in tqdm(range(self.nb_epochs)):
            # Hyperparams optimization using Simulated Annealing
            ...
            for batch in self.train_loader:
                ...
                # Forward pass
                train_loss, train_metrics = self.compute_forward_pass(batch, epoch, keep_grad=True)
                tmp_train_losses.append(train_loss.detach().data.cpu().numpy())

                # Updating the weights
                self.optimize_step(loss_value=train_loss)

                # Updating the metrics results of the current epoch
                if self.model_type.lower() == 'unet':
                    predictions_results['Train']['IoU'][epoch].append(train_metrics['IoU'])
                    predictions_results['Train']['DiceScore'][epoch].append(train_metrics['DiceScore'])
                else:
                    predictions_results['Train']['TrueLabels'][epoch] += train_metrics['TrueLabels']
                    predictions_results['Train']['PredictedLabels'][epoch] += train_metrics['PredictedLabels']
            ...
            if (self.separate_val_ds):
                with torch.no_grad():
                    ...
                    for batch in self.val_loader:
                        val_loss, val_metrics = self.compute_forward_pass(batch, epoch, keep_grad=False)
                        tmp_val_losses.append(val_loss.detach().data.cpu())

                        if self.model_type.lower() == 'unet':
                            predictions_results['Val']['IoU'][epoch].append(val_metrics['IoU'])
                            predictions_results['Val']['DiceScore'][epoch].append(val_metrics['DiceScore'])
                        else:
                            predictions_results['Val']['TrueLabels'][epoch] += val_metrics['TrueLabels']
                            predictions_results['Val']['PredictedLabels'][epoch] += val_metrics['PredictedLabels']
            ...
            with torch.no_grad():
                ...
                for batch in self.test_loader:
                    test_loss, test_metrics = self.compute_forward_pass(batch, epoch, keep_grad=False)
                    tmp_test_losses.append(test_loss.detach().data.cpu())

                    if self.model_type.lower() == 'unet':
                        predictions_results['Test']['IoU'][epoch].append(test_metrics['IoU'])
                        predictions_results['Test']['DiceScore'][epoch].append(test_metrics['DiceScore'])
                    else:
                        predictions_results['Test']['TrueLabels'][epoch] += test_metrics['TrueLabels']
                        predictions_results['Test']['PredictedLabels'][epoch] += test_metrics['PredictedLabels']
                ...
                if self.model_type.lower() == 'unet':
                    ...
                    train_iou = np.mean(predictions_results['Train']['IoU'][epoch])
                    train_dice = np.mean(predictions_results['Train']['DiceScore'][epoch])
                    print("\n=======>Train IoU at epoch {} is {}".format(epoch, train_iou))
                    print("\t\tTrain Dice Score at epoch {} is {}".format(epoch, train_dice))
                    
                    if (self.separate_val_ds):
                        val_iou = np.mean(predictions_results['Val']['IoU'][epoch])
                        val_dice = np.mean(predictions_results['Val']['DiceScore'][epoch])
                        print("\t\tVal IoU at epoch {} is {}".format(epoch, val_iou))
                        print("\t\tVal Dice Score at epoch {} is {}".format(epoch, val_dice))
                    
                    test_iou = np.mean(predictions_results['Test']['IoU'][epoch])
                    test_dice = np.mean(predictions_results['Test']['DiceScore'][epoch])
                    print("\t\tTest IoU at epoch {} is {}".format(epoch, test_iou))
                    print("\t\tTest Dice Score at epoch {} is {}".format(epoch, test_dice))
                    
                    # Save best model based on IoU
                    if test_iou > best_metric_so_far:
                        best_metric_so_far = test_iou
                        print(f"New best IoU: {best_metric_so_far}")
                        self.best_model = copy.deepcopy(self.model).cpu()
                else:
                    # Print classification metrics (original code)
                    ...
                    test_mcc = matthews_corrcoef(predictions_results['Test']['TrueLabels'][epoch], predictions_results['Test']['PredictedLabels'][epoch])
                    if test_mcc > best_metric_so_far:
                        best_metric_so_far = test_mcc
                        print(f"New best MCC: {best_metric_so_far}")
                        self.best_model = copy.deepcopy(self.model).cpu()

                print("================================================================================\n\n")

            # Sparsity calculations (unchanged)
