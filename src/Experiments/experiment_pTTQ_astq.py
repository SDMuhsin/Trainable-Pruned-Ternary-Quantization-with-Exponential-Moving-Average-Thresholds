...
#        return w_p*A + (-w_n*B)
    def quantize(self, kernel, w_p, w_n):
        ...
        pos_threshold = torch.quantile(pruned_kernel[pruned_kernel > 0], 0.5)
        neg_threshold = torch.quantile(pruned_kernel[pruned_kernel < 0], 0.5)
        
        # Adaptive quantization
        A = (pruned_kernel > pos_threshold).float()
        B = (pruned_kernel < neg_threshold).float()
        C = ((pruned_kernel <= pos_threshold) & (pruned_kernel > 0)).float()
        D = ((pruned_kernel >= neg_threshold) & (pruned_kernel < 0)).float()
        
        return w_p*A + (0.5*w_p)*C + (-w_n*B) + (-0.5*w_n)*D
    ...
    def get_grads(self, kernel_grad, kernel, w_p, w_n):
        ...
        grad_fp_kernel = w_p*A*kernel_grad + (0.5*w_p)*C*kernel_grad + (-w_n*B)*kernel_grad + (-0.5*w_n)*D*kernel_grad
        grad_wp = (A*kernel_grad).sum() + 0.5*(C*kernel_grad).sum()
        grad_wn = (B*kernel_grad).sum() + 0.5*(D*kernel_grad).sum()
