...
class Experiment(ExperimentBase):
            ...
            if (not self.countNonZeroParamsQuantizedLayers):
                ...
                if (self.model_to_use.lower() in ['mnist2dcnn','fmnist2dcnn']):
                    if ('conv' in name) and ('bias' not in name):
                        nonzero += torch.count_nonzero(param)
                elif (self.model_to_use.lower() in ['kmnistresnet18','fmnistresnet18','svhnresnet18','emnistresnet18']):
                    if ('conv' in name) and ('bias' not in name):
                        nonzero += torch.count_nonzero(param)
    ...
    def get_nb_params_to_quantize(self):
        ...
        for n, p in self.model.named_parameters():
            ...
            # Nb params quantize
            if (self.model_to_use.lower() in ['mnist2dcnn','fmnist2dcnn','kmnistresnet18','fmnistresnet18','svhnresnet18','emnistresnet18']):
