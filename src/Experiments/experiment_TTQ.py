            ...
            if (not self.countNonZeroParamsQuantizedLayers):
                ...
                if (self.model_to_use.lower() in ['mnist2dcnn','fmnist2dcnn']):
                    if ('conv' in name) and ('bias' not in name):
                        nonzero += torch.count_nonzero(param)
                elif (self.model_to_use.lower() in ['kmnistresnet18','fmnistresnet18','svhnresnet18','emnistresnet18','cifar10resnet50','cifar100resnet50','stl10resnet50','fmnistenet','kmnistdensenet']):
                    if ('conv' in name) and ('bias' not in name):
                        nonzero += torch.count_nonzero(param)
    ...
    def get_nb_params_to_quantize(self):
        ...
        for n, p in self.model.named_parameters():
            ...
            for val in p.shape:
                nb_params_layer *= val
            # Nb params quantize
            if (self.model_to_use.lower() in ['mnist2dcnn','fmnist2dcnn','kmnistresnet18','fmnistresnet18','svhnresnet18','emnistresnet18','cifar10resnet50','cifar100resnet50','stl10resnet50','fmnistenet','kmnistdensenet']):
