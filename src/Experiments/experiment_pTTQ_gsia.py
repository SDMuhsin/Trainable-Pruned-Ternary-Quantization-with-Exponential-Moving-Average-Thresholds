...
from src.utils.model_compression import approx_weights, approx_weights_fc, pruning_function_pTTQ, pruning_function_asymmetric_manessi, pruning_function_pTTQ_GSIA
...
class Experiment(ExperimentTTQ):
    def __init__(self, parameters_exp):
        ...
        elif (self.pruning_function_type.lower() == 'manessi_asymmetric_pttq_gsia'):
            self.pruning_function = pruning_function_pTTQ_GSIA
        else:
    ...
    def print_parent_parameters(self):
        def get_parent_attributes(cls, instance):
            for parent in cls.__mro__[1:]:  # Skip the current class
                print(f"\nParameters for {parent.__name__}:")
                # Class attributes
                class_attrs = {attr: getattr(parent, attr) for attr in dir(parent) 
                               if not attr.startswith("__") and not callable(getattr(parent, attr))}
                
                # Instance attributes (only for the immediate parent)
                if parent == cls.__base__:
                    instance_attrs = {attr: getattr(instance, attr) for attr in dir(instance) 
                                      if not attr.startswith("__") and not callable(getattr(instance, attr))
                                      and attr not in class_attrs}
                else:
                    instance_attrs = {}
                ...
                if class_attrs:
                    ...
                    print("  No non-callable class attributes found.")
                ...
                if instance_attrs:
                    ...
                    for attr, value in instance_attrs.items():
                        print(f"    {attr}: {value}")
                elif parent == cls.__base__:
                    print("  No non-callable instance attributes found.")
    ...
    def get_grads(self, kernel_grad, kernel, w_p, w_n):
        ...
        if (self.pruning_function_type == 'manessi_asymmetric_pTTQ_gsia'):
            # Grads of the thresholds hyperparameters
            kernel_mean, kernel_std = kernel.mean(), kernel.std()
            delta_min = (kernel_mean + self.a*kernel_std).abs()
            delta_max = (kernel_mean + self.b*kernel_std).abs()
