...
def extract_data(json_data):
    ...
    return np.array(sparsity), np.array(mcc), np.array(mcc_std)

def plot_pareto_frontier(sparsity, mcc, mcc_std, output_file):
    ...
    plt.errorbar(pareto_sparsity, pareto_frontier, yerr=pareto_std, fmt='o', color='b', label='Pareto Frontier', capsize=5)
...
def main():
    ...
    sparsity, mcc, mcc_std = extract_data(json_data)
    
    # Prepare output file name
    output_file = f"./results/pareto_frontier_{args.input_file.split('/')[-1].replace('.json', '')}.png"
    
    # Plot Pareto frontier
    plot_pareto_frontier(sparsity, mcc, mcc_std, output_file)
